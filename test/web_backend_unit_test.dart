...
import 'package:llamadart/src/backend/web/web_backend.dart';
import 'package:llamadart/src/backend/web/wllama_interop.dart';
...
void main() {
  group('WebLlamaBackend Unit', () {
    ...
    test('fallback chat template when utils missing', () async {
      // In JS interop we can't easily delete, but we can set to null if we change the mock setup
      // or just create a new backend instance for this test with a "broken" mock
      final brokenMock = JSObject();
      // Setup minimal required props for the backend to not crash on init
      brokenMock.setProperty(
        'loadModelFromUrl'.toJS,
        ((String url, JSAny? config) => Future.value(null).toJS).toJS,
      );
      brokenMock.setProperty(
        'getModelMetadata'.toJS,
        (([String? key]) => null).toJS,
      ...
      // No utils property set

      final brokenBackend = WebLlamaBackend(
        wllamaFactory: (pathConfig, [config]) => brokenMock as Wllama,
      );

      // Init to set _wllama
      await brokenBackend.modelLoadFromUrl('test', const ModelParams());

      final result = await brokenBackend.applyChatTemplate(1, [
    ...
    test('full lifecycle with mock', () async {
      await backend.modelLoad('http://mock/model.gguf', const ModelParams());
      expect(backend.isReady, isTrue);

      final tokens = await backend.tokenize(1, 'hello');
      expect(tokens, [1, 2, 3]);

      final text = await backend.detokenize(1, [1, 2, 3]);
      expect(text, 'decoded');

      final meta = await backend.modelMetadata(1);
      expect(meta['general.architecture'], 'llama');

      final result = await backend.applyChatTemplate(1, [
        const LlamaChatMessage(role: 'user', content: 'hi'),
      ]);
      expect(result.prompt, 'templated prompt');
      expect(result.stopSequences, contains('<|im_end|>'));
